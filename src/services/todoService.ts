...
class TodoService {
  ...
  // ❌ ENTFERNT: generateRecurringTodosForMonth()
  // Diese Funktion ist nicht mehr nötig, da Virtual Todos
  // clientseitig generiert werden durch useSmartTodos()
  ...
  // Alle RecurringTodos laden (für Virtual Todo Generation)
  async getAllRecurringTodos(): Promise<RecurringTodo[]> {
    const { data, error } = await supabase
    ...
    if (error) {
      console.error("Fehler beim Laden der RecurringTodos:", error.message);
      return [];
    }
